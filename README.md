...
-   简中游戏王卡片生成项目
-   因传播造成的版权问题与开发者无关
-   目前本项目已经足够完善，未来不会进行大规模改动(不包括上游)，可以放心使用. 除非 Konami 未来推出新的召唤法或者 pendulum-link/link-spell，否则本项目不会有 breaking changes，不过上游可能还会有 breaking changes

## 项目介绍

> 我认为很有必要说明本项目的意义所在，其实已经存在非常多的制卡器供大家选择，但是这些制卡器几乎都没有提供利于自动化的方式，其中绝大多数也并不开源甚至是死死防止他人借鉴（在这里我不想谈论这种行为的对错），所以才有了这个项目
>
> 但是我的需求足够少，因此我不会主动为这个项目引入过多的额外要素
>
> **这个项目不是为了 DIY 而存在的**
>
> 需要说明的是，使用这种方法生成卡图不太可能像 PS 一样精致
>
> 关于中心卡图，目前放弃了超分的想法
>
...
1. 衍生物信息来源于[Arshtyi/YuGiOh-Tokens](https://github.com/Arshtyi/YuGiOh-Tokens)
2. 种族翻译信息来源于[Arshtyi/Translations-Of-YuGiOh-Cards-Type](https://github.com/Arshtyi/Translations-Of-YuGiOh-Cards-Type)
3. 禁限卡表信息来源于[Arshtyi/YuGiOh-Forbidden-And-Limited-List](https://github.com/Arshtyi/YuGiOh-Forbidden-And-Limited-List)
...
5. 不会有高速魔法、黑暗同调、技能卡、卡背、RD 等，但是支持 Token
6. 不会支持异画(这牵涉到上下游的数据格式兼容问题)和非正式卡(如观赏卡)等
7. 所有卡片的描述采用 YGOPRO 风格
8. **禁限卡表支持且仅支持 OCG/TCG/MD**
...
10. container 采用 github actions 自动构建并发布于[packages](https://github.com/Arshtyi/YuGiOh-Cards-Maker/pkgs/container/yugioh-cards-maker)

## 关于 Token

-   衍生物信息来源于[Arshtyi/YuGiOh-Tokens](https://github.com/Arshtyi/YuGiOh-Tokens)仓库
-   目前 token 的 id 采用公认的召唤衍生物的卡的 id+1 的方式，具体信息见 `cfg.example/`和 `res/`
-   因为 token 的信息是**手动维护**，所以可能存在错误

## 关于 ID

因为本项目没有引入卡片编号的要素，所以使用了卡片密码作为 ID 以区别卡片（事实上这还是 Konami 的历史遗留问题）
...
-   `res/`:存放数据资源，已被切割为上游提供支撑
    ...
    -   `limit/`:禁限卡表
        ...
        -   `md.json`:md 禁限卡表
-   `script/`: 脚本目录
    -   `process_yugioh_cards.sh`: 处理数据
    -   `extract_ids.sh`:将 `res/token.json` 的所有卡片 id 压入 `dev/debug.txt`
    -   `extract_all_ids.sh`:将`tmp/cards.json`的所有卡片 id 压入`dev/debug.txt`
    -   `find_unofficial_cards.sh`:找出异画、非正式发售卡等卡片的 id
    -   `compare_debug_figure.sh`:比对 debug 结果
    -   `list_ref_images.sh`:将`ref/`下卡图文件名作为 id 写入`dev/debug.txt`
...
-   `tmp/`: 临时目录
    -   `cards.json`: 卡片数据
    -   `figure/`: 临时图片目录
-   `process_yugioh_cards.py`:由`script/progress_yugioh_cards.sh`调用，负责整合并处理数据得到 `tmp/cards.json`
-   `Program.cs`:卡图生成程序入口
